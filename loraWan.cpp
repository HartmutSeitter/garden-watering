...
// Credentials are entered via the web configuration portal (IotWebConf).
// Connect to the ESP hotspot "hs-garden-node-<chipid>", open 192.168.4.1,
// and enter DevEUI, AppEUI, AppKey from the TTN console.
// The necessary byte-reversal for DEVEUI and APPEUI is done in hex2data/reverseByteArray.
void os_getArtEui(u1_t *buf) {
  (void) hex2data(buf, APPEUI, 8);
  reverseByteArray(buf, 8);
}

void os_getDevEui(u1_t *buf) {
  (void) hex2data(buf, DEVEUI, 8);
  reverseByteArray(buf, 8);
}

void os_getDevKey(u1_t *buf) {
  (void) hex2data(buf, APPKEY, 16);
}
...
void onEvent(ev_t ev) {
  ...
  case EV_TXCOMPLETE:
    ...
    if (LMIC.dataLen) {
      log(DEBUG, "loraWan.cpp: Received %d bytes of payload", LMIC.dataLen);
