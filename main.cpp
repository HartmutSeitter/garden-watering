...
#define DEFAULT_MAX_PULSES_PER_INTERVAL 3000  // alarm if pulses in one read interval exceed this (kept in pulses)
...
void loop() {
  ...
  if ((millis() - check_on_off_time_timestamp) >= (CHECK_ON_OFF_TIME * 1000UL)) {
    ...
    // Rollierendes Display — wechselt mit jedem 5s-Tick
